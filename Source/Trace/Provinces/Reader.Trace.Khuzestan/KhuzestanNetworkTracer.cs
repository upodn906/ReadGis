...
namespace Reader.Trace.Khuzestan
{
    public class KhuzestanNetworkTracer : INetworkTracer
    {
        ...
        public KhuzestanNetworkTracer(IDbContextFactory<GisDbContext> gisContextFactory , 
            IDbContextFactory<KhuzestanDbContext> khuzestanContextFactory ,
            ILogger<KhuzestanNetworkTracer> logger)
        {
            _gisContextFactory = gisContextFactory;
            _khuzestanContextFactory = khuzestanContextFactory;
            _logger = logger;
        ...
        public async Task TraceAsync()
        {
            try
            {
                ...
                await using var gisCtx = await _gisContextFactory.CreateDbContextAsync();
                var feederObjectIds = gisCtx.Objects
                ...
                await using var truncateCtx = await _khuzestanContextFactory.CreateDbContextAsync();
                await truncateCtx.TruncateAsync<TraceResult>();
                foreach (var feederObjId in feederObjectIds)
                {
                    try
                    {
                        var traceResult = await GetFeederTraceAsync(feederObjId, token);
                        await using var khuCtx = await _khuzestanContextFactory.CreateDbContextAsync();
