...
public class WfsGeometryFactory : IGisGeometryFactory
{
    ...
    private List<Coordinate[]>? GetMultiLineStringCoordinate(JsonNode node)
    {
        ...
        {
            _logger.LogWarning("MultiLineString is not an array.");
            return null;
        }
        if (array.Count < 1)
        {
            _logger.LogWarning("MultiLineString contains less than 1 elements.");
            return null;
        }
        ...
        for (var i = 0; i < array.Count; i++)
        {
            ...
            if (line.Count < 1)
            {
                _logger.LogWarning("MultiLineString inner line contains less than 1 elements.");
                continue;
            }
