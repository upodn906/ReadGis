...
using Reader.Trace.Edsab;

namespace Reader.Trace.EdsabV2
{
    public static class EdsabTraceBootstrapper
    {
        public static void AddServices(IServiceCollection provider, IConfiguration configuration)
        {
            var config = configuration.GetSection("Edsab")?.Get<EdsabNetworkTracerConfig>();
            if (config == null)
                throw new InvalidOperationException("Edsab tracer config is not registered.");
            ...
            provider.AddSingleton<INetworkTracer, EdsabNetworkTracer>();
        }
    }
}
