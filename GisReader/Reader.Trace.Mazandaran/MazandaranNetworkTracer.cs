...
using Reader.Trace.Mazandaran.Models;
...
{
    public class MazandaranNetworkTracer : INetworkTracer
    {
        private readonly HttpClient _httpClient;
        private readonly GisDbContext _ctx;
        private readonly MazandaranNetworkTracerConfig _config;
        private readonly GisClientConfiguration _gisConfiguration;
        private readonly ILogger<MazandaranNetworkTracerConfig> _logger;
        static string token = string.Empty;

        public MazandaranNetworkTracer(GisDbContext ctx, MazandaranNetworkTracerConfig configuration, GisClientConfiguration gisConfiguration, ILogger<MazandaranNetworkTracerConfig> logger)
        {
        ...
        public async Task TraceAsync()
        {
            try
            {
                ...
                //}
                _logger.LogInformation("Token Not Required");
                var ids = _ctx.Objects
                    ...
                    .Select(Q => new { EdsId = Q.JNode!["SMID"]?.GetValue<int>(), CodeOf = Q.JNode["GISCODE"]?.GetValue<string>(), Q.ObjectId })
                    .ToList();
                ...
                foreach (var id in ids)
                {
                    ...
                    try
                    {
                        var traceResult = await GetFeederTraceAsync(id.CodeOf.ToString());
                        _logger.LogInformation($"Trace Result Code: {traceResult.ResultCode} with Message: {traceResult.ResultMessage} , {traceResult.Data.Count} , {traceResult.Edge.Count}");
                        ...
                        foreach (var layer in traceResult.Data.Where(x => x.GISCodes != null && x.GISCodes.Count() > 0 && !string.IsNullOrWhiteSpace(x.GISTableCode)))
                        {
                        ...
                        foreach (var layer in traceResult.Edge.Where(x => x.GISCodes != null && !string.IsNullOrWhiteSpace(x.GISTableCode)))
                        {
                            result.Add(new TraceResult
                            {
                                FeederObjectId = id.ObjectId,
        ...
        private async Task<TraceResponseModel> GetFeederTraceAsync(string feederId)
        {
            //string url = string.Empty;
            //if (_gisConfiguration.UseEsb)
            //    url = _config.Url;
            //else
            string url = $"{_config.Url}/graph/GetAnalysisResult";
            ...
            var content = new StringContent(JsonSerializer.Serialize(new
            {
                ...
                barriers = new[]
                {
                    new
                    {
                        gisTable = "FUSE_SWITCH",
                        gisCode = "FUSE_SWITCH_523279_4061206_1937870"
                    }
                },
                outputTables = new[] { "all" },
                outputType = 1
