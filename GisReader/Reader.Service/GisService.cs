...
namespace Reader.Service
{
    public class GisService : IGisService
    {
        ...
        public async Task<ServiceResult> ScanObjectsAsync()
        {
            return await TryScanAsync(async () =>
            {
                var layers = await GetLayersAndUpdateBaseDataAsync();
                var _ = Task.Run(async () => await ScanAsync(layers));
        ...
        private async Task ScanAsync(IReadOnlyList<GisLayer> layers)
        {
            if (layers.Count == 0)
            {
            ...
            _logger.LogInformation("Start gis scan with {threads} threads.", Configuration.Threads);
            ...
            try
            {
                _serviceReport.InitializeLayers(layers.Select(Q => new GisLayerStats(Q.Id,
                    Q.EnName)));
                var objectIds = await InitializeObjectIdsAsync(layers);
                ...
                await ProcessCodesAsync(objectIds);
        ...
        private async Task<ConcurrentDictionary<GisLayer, GisObjectIdsResult>>
            InitializeObjectIdsAsync(IReadOnlyList<GisLayer> codes)
        {
            ...
            await Parallel.ForEachAsync(
                codes, new ParallelOptions() { MaxDegreeOfParallelism = Configuration.Threads }, async (layer, _) =>
                {
                    try
                    {
        ...
        private async Task ProcessCodesAsync(ConcurrentDictionary<GisLayer, GisObjectIdsResult> ids)
        {
            var batches = CreateGisBatches(ids);
            if (Configuration.RandomizeBatches)
            {
                _logger.LogWarning("RandomizeBatches is TRUE and this cause out of sync layer reading.");
                RandomizeList(batches);
            }
            _logger.LogInformation("Created {count} gis batches.", batches.Count);
            await Parallel.ForEachAsync(
                batches, new ParallelOptions() { MaxDegreeOfParallelism = Configuration.Threads }, async (batch, _) =>
                {
                    try
                    {
        ...
        private List<KeyValuePair<(GisLayer Layer, string ObjectIdFiledName), IReadOnlyList<int>>> CreateGisBatches(
            ConcurrentDictionary<GisLayer, GisObjectIdsResult> ids)
        {
            var gisBatches = new List<KeyValuePair<(GisLayer Layer, string ObjectIdFiledName), IReadOnlyList<int>>>();
            foreach (var layer in ids)
            {
                if (layer.Value.Ids?.Count <= Configuration.BatchSize)
                {
                ...
                gisBatches.AddRange(layer.Value.Ids.OrderBy(Q => Q).Chunk(Configuration.BatchSize)
                    .Select(Q => new KeyValuePair<(GisLayer Layer, string ObjectIdFiledName), IReadOnlyList<int>>(
                        (layer.Key, layer.Value.FieldName), Q)));
            }
            return gisBatches.OrderBy(Q => Q.Key.Layer.Id).ToList();
        }
        private async Task<IReadOnlyList<GisLayer>> GetLayersAndUpdateBaseDataAsync()
        {
            await _client.InitializeAsync();
