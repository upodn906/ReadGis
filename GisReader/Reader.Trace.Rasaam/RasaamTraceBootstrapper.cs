...
using Reader.Trace.Rasaam;

namespace Reader.Trace.RasaamV2
{
    public static class RasaamTraceBootstrapper
    {
        public static void AddServices(IServiceCollection provider, IConfiguration configuration)
        {
            var config = configuration.GetSection("Rasaam")?.Get<RasaamNetworkTracerConfig>();
            if (config == null)
                throw new InvalidOperationException("Rasaam tracer config is not registered.");
            ...
            provider.AddSingleton<INetworkTracer, RasaamNetworkTracer>();
        }
    }
}
